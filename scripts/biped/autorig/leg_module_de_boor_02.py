...
class LegModule(object):
    ...
    def controllers_creation(self):
        ...
        for i, (name, guide) in enumerate(ik_controller_dict.items()):
            ...
            if i == 0:
                row_from_matrix_translate = cmds.createNode("rowFromMatrix", name=f"{self.side}_ankleIkTranslate_RFM", ss=True)
                cmds.setAttr(f"{row_from_matrix_translate}.input", 3)
                cmds.connectAttr(self.guides_matrices[2], f"{row_from_matrix_translate}.matrix")
                four_by_four_matrix = cmds.createNode("fourByFourMatrix", name=f"{self.side}_ankleIkTranslate_FFM", ss=True)
                cmds.connectAttr(f"{row_from_matrix_translate}.outputX", f"{four_by_four_matrix}.in30")
                cmds.connectAttr(f"{row_from_matrix_translate}.outputY", f"{four_by_four_matrix}.in31")
                cmds.connectAttr(f"{row_from_matrix_translate}.outputZ", f"{four_by_four_matrix}.in32")
                if self.side == "R":
                    cmds.setAttr(f"{four_by_four_matrix}.in00", -1)
                cmds.connectAttr(f"{four_by_four_matrix}.output", f"{ik_node[0]}.offsetParentMatrix")
            else:
                if child:
                    cmds.matchTransform(ik_node[0], guide, pos=True, rot=True)
                else:
                    mult_matrix_inverse_before_ctl = cmds.createNode("multMatrix", name=f"{self.side}_{name}Inverse_MMT", ss=True)
    ...
    def ik_setup(self):
        ...
        self.ik_matrices = custom_ik_solver.triangle_solver(name=f"{self.side}_legIk", guides=self.guides_matrices, controllers=[self.root_ik_ctl, self.pv_ctl, self.ik_controllers[0], self.ik_controllers[-1]], use_stretch=True, use_soft=True, ik_handle_manager=True, secondary_mode=(0, -1, 0) if self.side == 'L' else (0, 1, 0))
    ...
    def fk_stretch(self):
        ...
        subtract_upper = cmds.createNode("subtract", name=f"{self.side}_legFkUpperLength_SUB", ss=True)
        cmds.connectAttr(f"{self.fk_controllers[0]}.Stretch", f"{subtract_upper}.input1")
        cmds.setAttr(f"{subtract_upper}.input2", 1)

        subtract_lower = cmds.createNode("subtract", name=f"{self.side}_legFkLowerLength_SUB", ss=True)
    ...
    def de_boor_ribbon(self):
        ...
        ball_skinning_jnt = cmds.joint(name=f"{self.side}_legBallSkinning_JNT")
        cmds.connectAttr(f"{self.blend_matrices[-1]}.outputMatrix", f"{ball_skinning_jnt}.offsetParentMatrix")
        cmds.parent(ball_skinning_jnt, self.skeleton_grp)
        cmds.select(clear=True)
        ankle_skinning_jnt = cmds.joint(name=f"{self.side}_legAnkleSkinning_JNT")
